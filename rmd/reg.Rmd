...
* The adjusted R-squared value is 0.0057, indicating that only 0.6% of the variance in the net worth of billionaires can be explained by the variables in the model. This suggests that the factors we selected, have limited predictive power for a billionaire's net worth, there might be other factors that influence a billionaire's net worth that are not captured in this model.

* Multicollinearity check: Due to the low adjusted R-squared value, we checked for multicollinearity among the indicators by calculating their VIF (Variance Inflation Factor) values. All the indicators have VIF values near 1, indicating no significant correlation between any given indicator and the others and there is no multicollinearity issue in the model. In detail,"age", "gender", "gdp_country", "life_expectancy_country","self_made" have VIF values 1.011, 1.141, 1.076, 1.010, 1.208 respectively.

```{r vif}
vif_table <- vif(lrmodel)
vif_table <- data.frame(VIF = vif_table)
```


Predicotrs              | Vif value
----------------------- | ----------------------
          age           | 1.011
gender                  | 1.141
gdp_country             | 1.076
...
* For multiple linear regression models, one assumption is that the error term follows a normal distribution with zero mean and equal variance. According to the following Q-Q plot and density plot of the residuals, the residuals have a shape that is somewhat normal but are heavily right-skewed. This is a violation of one of the assumptions of linear regression, suggesting that the linear model may not be the best fit for this data.
...
*   In the following part, we give the plot of residuals versus fitted values and find some problems when using the multiple linear regression model to fit the data.
